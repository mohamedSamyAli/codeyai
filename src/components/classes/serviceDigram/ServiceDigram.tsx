...
export default memo<any>(({ data: { store, setNodes, setEdges, reactFlowWrapper, isConnectable, reactFlowInstance, customData, ...Data }, ...props }) => {
  ...
  const initComponentInTheStore  = ()=>{
    if (!store.current[props.id]) {
      ...
      store.current[props.id].componentType = DigramTypes.service
  ...
  return (
    <>
      <div onDrop={onDrop}>
        <div className={classNames('classdigram-container !border-[#d9ff00]',{shaded:props.selected})}>
          ...
          <div className='function flex flex-col relative !border-[#d9ff00]'>
            {properties.map(ele =>
