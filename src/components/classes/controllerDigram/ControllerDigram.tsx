...
export default memo<any>(({ data: { store, setNodes, setEdges, reactFlowWrapper, isConnectable, reactFlowInstance, customData, ...Data }, ...props }) => {
  ...
  const [title, setTitle] = useState("controllerName")
  ...
  const initComponentInTheStore = () => {
    if (!store.current[props.id]) {
      ...
      store.current[props.id].componentType = DigramTypes.controller
  ...
  useEffect(() => {
    ...
    if (customData) {
      ...
      if (customData?.functions) {
        let tempFunctions = customData?.functions.map((fn) => {
          return { ...fn, functionsCall: [] }
        }
        )
  ...
  return (
    <>
      <div onDrop={onDrop}>
        <div className={classNames('classdigram-container !border-[#ed9118]', { shaded: props.selected })}>
          ...
          <div className='function flex flex-col relative !border-[#ed9118]'>
            {properties.map(ele =>
